...
def _is_pvr_mode():
    return addon.getSetting('live_tv_mode') == 'PVR (recommended)'
...
def sync_pvr():
    if not _is_pvr_mode():
        xbmcgui.Dialog().notification('XStream Player', 'PVR sync not needed in Plugin list mode')
        return
    if not is_pvr_iptvsimple_installed():
        prompt_install_pvr()
        return
...
def main_menu():
    ...
    if 'live' in visible and has_live:
        if pvr_mode:
            ...
            items.append(('Live TV', {'mode': 'live_menu'}, 'DefaultAddonPVRClient.png'))
    if 'guide' in visible and pvr_mode and has_live:
    ...
    if len(watch_history.get_all()) > 0:
        items.append(('Recently Watched', {'mode': 'history_menu'}, 'DefaultRecentlyAddedEpisodes.png'))
...
def settings():
    ...
    if not _check_credentials_pin():
        return
    old_mode = addon.getSetting('live_tv_mode')
    addon.openSettings()
    _validate_settings()
    new_mode = addon.getSetting('live_tv_mode')
    if old_mode != new_mode:
        _log(f'Live TV mode changed: {old_mode} -> {new_mode}')
        if new_mode == 'PVR (recommended)':
            if not is_pvr_iptvsimple_installed():
                prompt_install_pvr()
                return
            pd = xbmcgui.DialogProgress()
            pd.create('XStream Player', 'Switching to PVR mode...')
            pd.update(20, 'Refreshing channel data...')
            try:
                _prefetch_all_data_silent()
            except Exception as e:
                _log(f'Prefetch error on mode switch: {e}')
            pd.update(60, 'Syncing to PVR...')
            try:
                _sync_pvr_force()
            except Exception as e:
                _log(f'PVR sync error on mode switch: {e}')
            pd.close()
            xbmcgui.Dialog().notification('XStream Player', 'PVR mode activated')
        else:
            xbmcgui.Dialog().notification('XStream Player', 'Plugin list mode activated')
        xbmc.executebuiltin('Container.Refresh')
