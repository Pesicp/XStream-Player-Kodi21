...
def _cache_path(name):
    profile = xbmcvfs.translatePath(addon.getAddonInfo('profile'))
    return os.path.join(profile, f'data_cache_p{pm.active}_{name}.json')
...
def _is_adult_category(name):
    adult_keywords = [
        'xxx', 'adult', '18+', 'mature', 'porn', 'erotic', 'erotica',
        'nsfw', 'x-rated', 'xrated', 'playboy', 'hustler', 'penthouse',
        'brazzers', 'naughty', 'sexy', 'sex ', 'hentai', 'stripte',
        'fetish', 'nude', 'naked', 'orgasm', 'kamasutra', 'milf',
        'lesbian', 'gay ', 'hardcore', 'softcore', 'topless',
        'uncensored', 'uncut', 'voyeur', 'swinger', 'redlight',
        'red light', 'blue film', 'hot club', 'night show',
        'for adults', '+18', '(18+)', '[18+]',
    ]
    lower = name.lower()
...
def refresh_data():
    ...
    pd = xbmcgui.DialogProgress()
    pd.create('XStream Player', 'Clearing cache...' if clear_cache else 'Refreshing data...')
    ...
    choice = xbmcgui.Dialog().yesno(
        'XStream Player',
        'Refresh complete! Restart Kodi to apply all changes.',
...
def manage_content_dialog(stype, profile_num):
    ...
    while True:
        ...
        if choice == 0:
            ...
            cat_names = ['[Select All]', '[Deselect All]'] + [c.get('category_name', 'Unknown') for c in cats]
            ...
            if 1 in result:
                ...
                result = dialog.multiselect(f'Check categories to HIDE', cat_names, preselect=[])
                if result is None:
                    continue
        ...
        names = ['[Select All]', '[Deselect All]'] + [s.get('name', 'Unknown') for s in filtered]
        ...
        if 0 in result:
            ...
            if result is None:
                continue
        if 1 in result:
            result = dialog.multiselect(f'Check items to HIDE in {cat_name}', names, preselect=[])
            if result is None:
                continue
        real_result = [i - 2 for i in result if i >= 2] if result else []
        cat_id_set = set(ids)
...
def manage_hidden_subcats(stype, profile_num=None):
    ...
    li = xbmcgui.ListItem(label='[COLOR red]Hide All Categories[/COLOR]')
    li.setArt({'icon': 'DefaultAddonNone.png'})
    q = {'mode': 'hide_all_subcats', 'stype': stype, 'pnum': pnum, 'action': 'hide'}
    xbmcplugin.addDirectoryItem(handle=addon_handle, url=build_url(q), listitem=li, isFolder=False)
    li = xbmcgui.ListItem(label='[COLOR green]Unhide All Categories[/COLOR]')
    li.setArt({'icon': 'DefaultAddonNone.png'})
    ...
    if choice == 0:
        ...
        if 0 in result:
            ...
            if result is None:
                return
        if 1 in result:
            result = dialog.multiselect(f'Select items to HIDE in {cat_name}', names, preselect=[])
            if result is None:
                return
        real_result = [i - 2 for i in result if i >= 2] if result else []
        cat_ids = set(ids)
...
def hide_all_subcats(stype, action, profile_num=None):
    ...
    if action == 'hide':
        ...
        _set_hidden_subcats(stype, new_hidden, pnum)
        xbmcgui.Dialog().notification('XStream Player', f'Hidden all {len(new_hidden)} categories')
    else:
        _set_hidden_subcats(stype, set(), pnum)
        xbmcgui.Dialog().notification('XStream Player', 'All categories unhidden')
    ...
    for label, url in [('Xtream Server URL', xt_url), ('M3U URL', m3u_url)]:
        if url and not url.startswith(('http://', 'https://')):
            xbmcgui.Dialog().notification('XStream Player',
                ...
                xbmcgui.NOTIFICATION_WARNING, 5000)
        elif url and url.startswith('http://') and addon.getSetting('warn_http').lower() == 'true':
            xbmcgui.Dialog().notification('XStream Player',
                f'{label} uses insecure HTTP',
...
def clear_cache_menu():
    ...
    if choice == 0:
        ...
        if dialog.yesno('XStream Player', 'Clear all watch history?'):
            watch_history.clear()
            dialog.notification('XStream Player', 'Watch history cleared')
...
if mode is None:
    ...
    if fname:
        new_name = xbmcgui.Dialog().input('Rename group', default=fname)
    ...
    dialog = xbmcgui.Dialog()
    new_name = dialog.input('Rename PVR Group', default=old_name)
    ...
    pd = xbmcgui.DialogProgress()
    pd.create('XStream Player', 'Reloading EPG...')
    EPG(addon).fetch()
    pd.close()
    xbmcgui.Dialog().notification('XStream Player', 'EPG reloaded')
elif mode == 'force_reload_pvr':
    pd = xbmcgui.DialogProgress()
    pd.create('XStream Player', 'Syncing PVR...')
    _sync_pvr_force()
    pd.close()
    xbmcgui.Dialog().notification('XStream Player', 'PVR synced')
