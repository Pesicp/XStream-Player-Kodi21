...
def manage_content_dialog(stype, profile_num):
    ...
    while True:
        ...
        # Build "Hidden Items" count
        hidden_item_count = len(hidden_items)
        labels = [f'[COLOR gold]Hidden Items ({hidden_item_count})[/COLOR]']
        for c in cats:
            cat_id = str(c.get('category_id', ''))
            cat_name = c.get('category_name', 'Unknown')
            prefix = '[COLOR yellow][HIDDEN][/COLOR] ' if cat_id in hidden else ''
            labels.append(f'{prefix}{cat_name}')
        choice = dialog.select(f'Manage {stype_label} content', labels)
        if choice < 0:
            break
        if choice == 0:
            # Hidden Items: show all individually hidden items as multiselect to unhide
            if not hidden_items:
                dialog.notification('XStream Player', 'No hidden items')
                continue
            # Resolve names from stream data
            ...
            # All pre-selected (checked = keep hidden)
            ...
            if result is None:
                continue
            # Keep only checked items hidden
            new_hidden = {h_ids[i] for i in result} if result else set()
            # Preserve any hidden IDs not in current stream data
        ...
        if is_hidden:
            actions = ['Unhide entire category', 'View content inside']
        else:
            actions = ['Hide entire category', 'View content inside']
        action = dialog.select(cat_name, actions)
        if action < 0:
            continue
        if action == 0:
            if is_hidden:
                hidden.discard(cat_id)
                dialog.notification('XStream Player', f'Unhidden: {cat_name}')
            else:
                hidden.add(cat_id)
                dialog.notification('XStream Player', f'Hidden: {cat_name}')
